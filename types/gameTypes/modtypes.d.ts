declare namespace Modding {
    ...
    type ClassPatches<C extends ClassHandle> = Map<
        ClassMethod<C> | ClassProperty<C>,
        {
            mods: Set<Mod>;
    ...
    type GetterPatch<C extends ClassHandle, P extends ClassProperty<C>> = (
        this: InstanceType<C>,
        o: () => C['prototype'][P]
    ) => C['prototype'][P];
    type SetterPatch<C extends ClassHandle, P extends ClassProperty<C>> = (
        this: InstanceType<C>,
        o: (val: C['prototype'][P]) => void,
        val: C['prototype'][P]
    ) => void;

    type AnyFunction = (...args: any) => any;
    type BeforePatch<C extends ClassHandle, M extends AnyFunction> = (
        this: InstanceType<C>,
        ...args: Parameters<M>
    ) => Parameters<M> | void;
    type AfterPatch<C extends ClassHandle, M extends AnyFunction> = (
        this: InstanceType<C>,
        returnValue: ReturnType<M>,
        ...args: Parameters<M>
    ) => ReturnType<M> | void;
    type ReplacePatch<C extends ClassHandle, M extends AnyFunction> = (
        this: InstanceType<C>,
        replacedMethod: (...args: Parameters<M>) => ReturnType<M>,
        ...args: Parameters<M>
    ) => ReturnType<M>;

    type MethodOrPropertyPatch<
        C extends ClassHandle,
        T extends ClassMethod<C> | ClassProperty<C>
    > = T extends ClassMethod<C> ? MethodPatch<C, T> : T extends ClassProperty<C> ? PropertyPatch<C, T> : never;
    ...
    interface ModContext {
        ...
        patch: <C extends ClassHandle, MorP extends ClassMethod<C> | ClassProperty<C>>(
            _class: C,
            methodOrPropertyName: MorP
        ) => MethodOrPropertyPatch<C, MorP>;
        isPatched: <C extends ClassHandle>(
            _class: C,
            methodOrPropertyName: ClassMethod<C> | ClassProperty<C>
        ) => boolean;
    }
    ...
    type GameDataPackageBuilderProperties = ToGameDataAdders<FlattenProps<Required<GameData>>> &
        ToGameDataModifiers<FlattenProps<Required<GameDataModifications>>>;

    interface GameDataPackageBuilder extends GameDataPackageBuilderProperties {
        skills: {
            add(
                _class: new (namespace: DataNamespace, game: Game) => T &
                    Partial<PassiveAction> &
                    Partial<ActiveAction> &
                    Partial<StatProvider>
            ): void;
        };
    }
}
...
type ClassMethod<C extends ClassHandle> = {
    [M in keyof C['prototype']]-?: C['prototype'][M] extends Function ? M : never;
}[keyof C['prototype']];

type ClassProperty<C extends ClassHandle> = {
    [P in keyof C['prototype']]-?: C['prototype'][P] extends Function ? never : P;
}[keyof C['prototype']];
